...
//---------------------------------------------------------------------------------------------------------
void ofxSphericCamera::setup( string theFileName ){
...
string ofxSphericCamera::load( string theFileName ){
	...
	if( !fileIn.is_open() ){
		fileIn.close();
		return "ofxSphericCamera ERROR: Couldn't load file \n";
	}
	...
	if( dataName != "version=" ){
		fileIn.close();
		return "ofxSphericCamera ERROR: Bad project Info file format loading version\n";
	}
	...
	if( dataValueString == "0.1" ){
		...
		if( dataName != "cameraAngle=" ){
			fileIn.close();
			return "ofxSphericCamera ERROR: Bad project Info file format loading cameraAngle\n";
		}
		fileIn >> cameraAngle;
		...
		if( dataName != "cameraFar=" ){
			fileIn.close();
			return "ofxSphericCamera ERROR: Bad project Info file format loading cameraFar\n";
		}
		fileIn >> cameraFar;
		...
		if( dataName != "cameraNear=" ){
			fileIn.close();
			return "ofxSphericCamera ERROR: Bad project Info file format loading cameraNear\n";
		}
		fileIn >> cameraNear;
		...
		if( dataName != "cameraCenter=" ){
			fileIn.close();
			return "ofxSphericCamera ERROR: Bad project Info file format loading cameraCenter\n";
		}
		...
		if( dataName != "cameraUp=" ){
			fileIn.close();
			return "ofxSphericCamera ERROR: Bad project Info file format loading cameraUp\n";
		}
		...
		if( dataName != "cameraRadius=" ){
			fileIn.close();
			return "ofxSphericCamera ERROR: Bad project Info file format loading cameraRadius\n";
		}
		fileIn >> cameraRadius;
		...
		if( dataName != "cameraTeta=" ){
			fileIn.close();
			return "ofxSphericCamera ERROR: Bad project Info file format loading cameraTeta\n";
		}
		fileIn >> cameraTeta;
		...
		if( dataName != "cameraFi=" ){
			fileIn.close();
			return "ofxSphericCamera ERROR: Bad project Info file format loading cameraFi\n";
		}
		fileIn >> cameraFi;
		...
		fileIn.close();
		return "ofxSphericCamera ERROR: Bad file version\n";
	}
}
//---------------------------------------------------------------------------------------------------------
string ofxSphericCamera::save(  string theFileName  ){
	string path = "cameras/";
	fileName = path + theFileName;
	fileName = ofToDataPath( fileName );
	...
	if( !fileOut.is_open() ){
		fileOut.close();
		return "ofxSphericCamera ERROR: Couldn't save file\n";
	}
...
//--------------------------------------------------------------
void ofxSphericCamera::draw( int x , int y , int width , int height ){ 
...
//--------------------------------------------------------------
void ofxSphericCamera::setDeltaCameraSphericalCoordinatesRadius( float theDeltaRadius ){
	cameraDeltaRadius	= theDeltaRadius;
	haveToUpdateCamera	= true;
}
//--------------------------------------------------------------
void ofxSphericCamera::setDeltaCameraSphericalCoordinatesTeta( float theDeltaTeta ){
	cameraDeltaTeta		= theDeltaTeta;
	haveToUpdateCamera	= true;
}
//--------------------------------------------------------------
void ofxSphericCamera::setDeltaCameraSphericalCoordinatesFi( float theDeltaFi ){
	cameraDeltaFi		= theDeltaFi;
	haveToUpdateCamera	= true;
}
//--------------------------------------------------------------
float ofxSphericCamera::getCameraRadius( ){
	return cameraRadius;
}
//--------------------------------------------------------------
float ofxSphericCamera::getCameraTeta( ){
	return cameraTeta;
}
//--------------------------------------------------------------
float ofxSphericCamera::getCameraFi( ){
	return cameraFi;
}   
//--------------------------------------------------------------
void ofxSphericCamera::setCameraRadius( float theRadius ){
	cameraRadius = theRadius;
}
//--------------------------------------------------------------
void ofxSphericCamera::setCameraTeta( float theTeta ){
	cameraTeta = theTeta;
}
//--------------------------------------------------------------
